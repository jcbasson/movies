...
function validateOperation(operation) {
    ...
    for (var _i = 0, _a = Object.keys(operation); _i < _a.length; _i++) {
        ...
        if (OPERATION_FIELDS.indexOf(key) < 0) {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(16) : new tsInvariant.InvariantError("illegal argument: " + key);
        }
    }
    return operation;
}
...
function checkDocument(doc) {
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(doc && doc.kind === 'Document', 17) : tsInvariant.invariant(doc && doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    var operations = doc.definitions
        .filter(function (d) { return d.kind !== 'FragmentDefinition'; })
        .map(function (definition) {
        if (definition.kind !== 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(18) : new tsInvariant.InvariantError("Schema type definitions not allowed in queries. Found: \"" + definition.kind + "\"");
        }
        return definition;
    });
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(operations.length <= 1, 19) : tsInvariant.invariant(operations.length <= 1, "Ambiguous GraphQL document: contains " + operations.length + " operations");
    return doc;
}
...
function getQueryDefinition(doc) {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(queryDef && queryDef.operation === 'query', 20) : tsInvariant.invariant(queryDef && queryDef.operation === 'query', 'Must contain a query definition.');
    return queryDef;
}
function getFragmentDefinition(doc) {
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(doc.kind === 'Document', 21) : tsInvariant.invariant(doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(doc.definitions.length <= 1, 22) : tsInvariant.invariant(doc.definitions.length <= 1, 'Fragment must have exactly one definition.');
    var fragmentDef = doc.definitions[0];
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragmentDef.kind === 'FragmentDefinition', 23) : tsInvariant.invariant(fragmentDef.kind === 'FragmentDefinition', 'Must be a fragment definition.');
    return fragmentDef;
}
function getMainDefinition(queryDoc) {
    ...
    throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(24) : new tsInvariant.InvariantError('Expected a parsed GraphQL query with a query, mutation, subscription, or a fragment.');
}
...
var ApolloLink = (function () {
    ...
    ApolloLink.prototype.request = function (operation, forward) {
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(5) : new tsInvariant.InvariantError('request is not implemented');
    };
...
function assertNotCacheFirstOrOnly(obsQuery) {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(fetchPolicy !== 'cache-first' && fetchPolicy !== 'cache-only', 4) : tsInvariant.invariant(fetchPolicy !== 'cache-first' && fetchPolicy !== 'cache-only', 'Queries that specify the cache-first and cache-only fetchPolicies cannot also be polling queries.');
}
...
var serializeFetchParameter = function (p, label) {
    ...
    try {
        ...
        var parseError = process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(6) : new tsInvariant.InvariantError("Network request failed. " + label + " is not serializable: " + e.message);
...
var checkFetcher = function (fetcher) {
    if (!fetcher && typeof fetch === 'undefined') {
        ...
        if (typeof window === 'undefined')
            library = 'node-fetch';
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(7) : new tsInvariant.InvariantError('"fetch" has not been found globally and no fetcher has been ' +
...
function shouldInclude(_a, variables) {
    ...
    return getInclusionDirectives(directives).every(function (_a) {
        ...
        if (ifArgument.value.kind === 'Variable') {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(evaledValue !== void 0, 44) : tsInvariant.invariant(evaledValue !== void 0, "Invalid variable referenced in @" + directive.name.value + " directive.");
        }
        else {
...
function getInclusionDirectives(directives) {
    ...
    if (directives && directives.length) {
        directives.forEach(function (directive) {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(directiveArguments && directiveArguments.length === 1, 45) : tsInvariant.invariant(directiveArguments && directiveArguments.length === 1, "Incorrect number of arguments for the @" + directiveName + " directive.");
            var ifArgument = directiveArguments[0];
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(ifArgument.name && ifArgument.name.value === 'if', 46) : tsInvariant.invariant(ifArgument.name && ifArgument.name.value === 'if', "Invalid argument for the @" + directiveName + " directive.");
            var ifValue = ifArgument.value;
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(ifValue &&
                (ifValue.kind === 'Variable' || ifValue.kind === 'BooleanValue'), 47) : tsInvariant.invariant(ifValue &&
                ...
                execute = function (selection) { return tslib.__awaiter(_this, void 0, void 0, function () {
                    ...
                    return tslib.__generator(this, function (_a) {
                        ...
                        if (isInlineFragment(selection)) {
                            ...
                            process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragment, 15) : tsInvariant.invariant(fragment, "No fragment named " + selection.name.value);
                        }
...
var QueryStore = (function () {
    ...
    QueryStore.prototype.initQuery = function (query) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(!previousQuery ||
            ...
            equality.equal(previousQuery.document, query.document), 48) : tsInvariant.invariant(!previousQuery ||
            previousQuery.document === query.document ||
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.stop = function () {
        ...
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(25) : new tsInvariant.InvariantError('QueryManager stopped while query was in flight'));
        });
    };
    ...
    QueryManager.prototype.watchQuery = function (options, shouldSubscribe) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(options.fetchPolicy !== 'standby', 28) : tsInvariant.invariant(options.fetchPolicy !== 'standby', 'client.watchQuery cannot be called with fetchPolicy set to "standby"');
    ...
    QueryManager.prototype.query = function (options) {
        var _this = this;
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(options.query, 29) : tsInvariant.invariant(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.');
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(options.query.kind === 'Document', 30) : tsInvariant.invariant(options.query.kind === 'Document', 'You must wrap the query string in a "gql" tag.');
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(!options.returnPartialData, 31) : tsInvariant.invariant(!options.returnPartialData, 'returnPartialData option only supported on watchQuery.');
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(!options.pollInterval, 32) : tsInvariant.invariant(!options.pollInterval, 'pollInterval option only supported on watchQuery.');
        return new Promise(function (resolve, reject) {
    ...
    QueryManager.prototype.clearStore = function () {
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(33) : new tsInvariant.InvariantError('Store reset while query was in flight (not completed in link chain)'));
        });
        var resetIds = [];
    ...
    QueryManager.prototype.getQueryWithPreviousResult = function (queryIdOrObservable) {
        ...
        if (typeof queryIdOrObservable === 'string') {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(foundObservableQuery, 34) : tsInvariant.invariant(foundObservableQuery, "ObservableQuery with this id doesn't exist: " + queryIdOrObservable);
    ...
    QueryManager.prototype.startPollingQuery = function (options, queryId, listener) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(pollInterval, 35) : tsInvariant.invariant(pollInterval, 'Attempted to start a polling query without a polling interval.');
        if (!this.ssrMode) {
...
var Policies = (function () {
    ...
    Policies.prototype.setRootTypename = function (which, typename) {
        ...
        if (typename !== old) {
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(!old || old === which, 37) : tsInvariant.invariant(!old || old === which, "Cannot change root " + which + " __typename more than once");
    ...
    Policies.prototype.readField = function (objectOrReference, nameOrField, context, typename) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(objectOrReference, 38) : tsInvariant.invariant(objectOrReference, "Must provide an object or Reference when calling Policies#readField");
        var policies = this;
    ...
    return {
        ...
        mergeObjects: function (existing, incoming) {
            if (Array.isArray(existing) || Array.isArray(incoming)) {
                throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(39) : new tsInvariant.InvariantError("Cannot automatically merge arrays");
            }
...
function computeKeyObject(response, specifier, aliasMap) {
    ...
    specifier.forEach(function (s) {
        if (Array.isArray(s)) {
            ...
            process.env.NODE_ENV === "production" ? tsInvariant.invariant(hasOwn.call(response, responseName), 40) : tsInvariant.invariant(hasOwn.call(response, responseName), "Missing field " + responseName + " while computing key fields");
...
var StoreReader = (function () {
    ...
    StoreReader.prototype.executeSelectionSet = function (_a) {
        ...
        if (isReference(objectOrReference) &&
            ...
            return {
                result: {},
                missing: [process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(51) : new tsInvariant.InvariantError("Dangling reference to missing " + objectOrReference.__ref + " object")],
            };
        }
            ...
            if (isField(selection)) {
                ...
                if (fieldValue === void 0) {
                    if (!addTypenameToDocument.added(selection)) {
                        getMissing().push(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(52) : new tsInvariant.InvariantError("Can't find field " + selection.name.value + " on " + (isReference(objectOrReference)
                ...
                if (isInlineFragment(selection)) {
                    ...
                    process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragment = fragmentMap[selection.name.value], 53) : tsInvariant.invariant(fragment = fragmentMap[selection.name.value], "No fragment named " + selection.name.value);
                }
    ...
    if (!field.selectionSet) {
        ...
        workSet_1.forEach(function (value) {
            if (value && typeof value === "object") {
                process.env.NODE_ENV === "production" ? tsInvariant.invariant(!isReference(value), 54) : tsInvariant.invariant(!isReference(value), "Missing selection set for object of type " + getTypenameFromStoreObject(store, value) + " returned for query field " + field.name.value);
...
function parser(document) {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(!!document && !!document.kind, 11) : tsInvariant.invariant(!!document && !!document.kind, "Argument of " + document + " passed to parser was not a valid GraphQL " +
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), 12) : tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), "Passing only a fragment to 'graphql' is not yet supported. " +
        "You must include a query, subscription or mutation as well");
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, 13) : tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, "react-apollo only supports a query, subscription, or a mutation per HOC. " +
    ...
    var definitions = queries.length
        ...
        : mutations.length
            ? mutations
            : subscriptions;
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(definitions.length === 1, 14) : tsInvariant.invariant(definitions.length === 1, "react-apollo only supports one definition per HOC. " + document + " had " +
